...
esp_err_t handle_ws_req(httpd_req_t *req) {
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT) {
    	...
    	if (strcmp(in_type, "status_sync") == 0) {
			...
			if (type_obj->valueint == 0) {
				set_animation(&hsli, in_id, part_id, 254, speed_obj->valueint, brightness_obj->valueint, true);
			}
			else {
				set_animation(&hsli ,in_id, part_id, type_obj->valueint, speed_obj->valueint, brightness_obj->valueint, true);
			}
...
esp_err_t handle_wss_req(httpd_req_t *req)
{
    ...
    // Nejdříve se přijme celá WS zpráva
    // Nastavím max_len = 0 abych dostal délku zprávy
    ...
    // Pokud je zpráva PONG, aktualizuj keep-alive
    if (ws_pkt.type == HTTPD_WS_TYPE_PONG) {
		...
		// Odpověď na CLOSE paket zprávou bez jakéhokoliv obsahu
		ws_pkt.len = 0;
		ws_pkt.payload = NULL;
        ret = httpd_ws_send_frame(req, &ws_pkt);
        if (ret != ESP_OK) {
		...
		if (strcmp(in_type, "login") == 0) {
			...
			if (strcmp(pass_obj->valuestring, user_config->admin_pass) == 0){
				...
				cJSON_AddNumberToObject(debugdata, "nol", lights_.nol);
				cJSON_AddItemToObject(debugdata, "seg", segments=cJSON_CreateObject());
				char index_string[5];
				for (int i = 0; i < lights_.nol; i++) {
					sprintf(index_string, "%d", i+1);
					cJSON_AddNumberToObject(segments, index_string , get_amt_of_segs(&hsli,i));
				}
			}
			else {
			...
			if (strcmp(pass_obj->valuestring, user_config->admin_pass) == 0){
				...
				if (!check_json_object(state_obj,cJSON_True)) {
					set_segment(&hsli, light_obj->valueint-1, index_obj->valueint-1, 50);
				}
				else if (!check_json_object(state_obj,cJSON_False)) {
					set_segment(&hsli, light_obj->valueint-1, index_obj->valueint-1, false);
				}
				else {
					FREE_AND_RETURN_FAIL
				}
			...
			if (strcmp(pass_obj->valuestring, user_config->admin_pass) == 0){
				...
				set_number_of_active_lights(&hsli,num_lights_obj->valueint);
				...
				for (int i = 0; i < cJSON_GetArraySize(lights); i++) {
					...
					set_amount_of_drivers(&hsli,i, num_drivers_obj->valueint);
					...
					if (drivers != NULL) {
						for (int j = 0; j < cJSON_GetArraySize(drivers); j++) {
							...
							make_wiring_mask(&hsli,i, j, mask_bin);
						}
					}
					...
					if (parts != NULL) {
						for (int j = 0; j < cJSON_GetArraySize(parts); j++) {
							...
							set_light_part(&hsli, i, j, start_obj->valueint, length_obj->valueint);
						}
					}
				}
